/** @module ext.confirmEdit.CaptchaWidget */
mw.libs.confirmEdit = mw.libs.confirmEdit || {};

/**
 * Configuration options for the CAPTCHA widget provided when calling the constructor of
 * {@link CaptchaWidget}.
 *
 ...
 *   in the DOM before calling {@link CaptchaWidget#renderCaptcha}.
 * @property {string} [type] The type of CAPTCHA to render. Can be set later via
 *   {@link CaptchaWidget#updateForFailure} if not known at the time of construction.
 * @property {string} [interfaceName] The name of the interface where the action is being
 *   performed, used for analytics.
 * @property {boolean} [showLoadingIndicator] Only relevant when the CAPTCHA type is hCaptcha.
 *   If `true`, then a loading indicator is shown while hCaptcha runs its checks.
 *   Off by default; enable for interfaces which have no loading animation during save
 *   (e.g. UploadWizard) so they do not appear frozen during the wait.
 */

/**
 * The CAPTCHA data to be submitted in an API request by appending all fields to the request.
 * Returned by {@link CaptchaWidget#getCaptchaDataForSubmission}.
 *
 * @typedef CaptchaSubmissionData
 * @property {string} captchaid The captcha ID field to submit, may be empty for some CAPTCHAs.
 * @property {string} captchaword The captcha word field to submit. Depending on the CAPTCHA type,
 *   this may be user-entered data or a token generated by the CAPTCHA.
 * @property {boolean} [wgConfirmEditForceShowCaptcha] An optional field, appended to
 *   indicate that the CAPTCHA challenge solved was the one force-shown. For example,
 *   set when the user matches an AbuseFilter filter with the "showcaptcha" consequence.
 */

/**
 * @class
 *
 * This class is used to render a CAPTCHA and get the CAPTCHA data for use in an API request.
 * It supports hCaptcha, FancyCaptcha, and question-based CAPTCHAs.
 *
 * @alias CaptchaWidget
 * @constructor
 * @param {CaptchaWidgetConfig} [config] Configuration options for the CAPTCHA widget
 * @property {CaptchaWidgetConfig} config
 * @property {string} captchaWord
 * @property {string} captchaId
 * @property {boolean} captchaRendered Whether the CAPTCHA has been rendered
 * @property {boolean} alreadyRecommendedResubmit Whether auto-resubmit has already been recommended
 * @property {string} hCaptchaSiteKey
 * @property {string} hCaptchaWidgetId
 * @property {boolean} hCaptchaForceShowCaptcha Whether the hCaptcha challenge was force-shown
 *   (e.g. due to the user matching an AbuseFilter filter with the "showcaptcha" consequence).
 * @property {boolean} hCaptchaShowResubmitMessage Whether to show a resubmit guidance message
 * @property {string} captchaQuestion
 * @property {string} captchaQuestionMime
 * @property {string} captchaImageUrl
 * @property {HTMLDivElement|null} captchaInputField Input field container for
 *   CAPTCHAs that need an input field
 */
 ...
 * API request.
 *
 * @alias CaptchaWidget.static.captchaNeededForEdit
 * @return {false|string}
 */
 ...
 *
 * @alias CaptchaWidget.prototype.renderCaptcha
...
mw.libs.confirmEdit.CaptchaWidget.prototype.renderHCaptcha = function ( $captchaContainer ) {
	// A new hCaptcha widget will generate a new hCaptcha token,
	// so rerendering needs to clear this out
	this.captchaWord = '';
 ...
 * an `input` element)
 *
 * @alias CaptchaWidget.prototype.getInputField
 ...
 *
 * @alias CaptchaWidget.prototype.getCaptchaDataForSubmission
 ...
 *
 * You can call this method before or after calling {@link CaptchaWidget#renderCaptcha}.
 * If the CAPTCHA is already rendered, calling this method may re-render the CAPTCHA.
 *
 * @alias CaptchaWidget.prototype.updateForFailure
 ...
 *   {@link CaptchaWidget#getCaptchaDataForSubmission}.
 */
